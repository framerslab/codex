...
echo -e "${BLUE}╔════════════════════════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║  Frame Codex Full Re-Catalog & Metadata Update            ║${NC}"
echo -e "${BLUE}╚════════════════════════════════════════════════════════════╝${NC}"
echo ""

# Check if we're in the right directory
if [ ! -f "package.json" ] || [ ! -d "weaves" ]; then
  echo -e "${RED}❌ Error: Must run from apps/codex directory${NC}"
  exit 1
fi
...
echo -e "${BLUE}🔍 Running full catalog analysis...${NC}"
...
if [ "$DRY_RUN" = true ]; then
  ...
  echo ""
  echo -e "${GREEN}✓${NC} Dry run complete. Check codex-report.json for analysis."
  exit 0
fi
...
if git diff --quiet && git diff --cached --quiet; then
  echo -e "${YELLOW}ℹ️  No changes detected. Index is up to date.${NC}"
  git checkout main
...
# Push branch
echo ""
echo -e "${BLUE}⬆️  Pushing branch...${NC}"
...
if [ -z "$GH_PAT" ]; then
  echo -e "${YELLOW}⚠️  GH_PAT not set. Please create PR manually:${NC}"
  echo ""
...
if [ "$AUTO_MERGE" = "true" ]; then
  echo ""
  echo -e "${YELLOW}🤖 Auto-merge enabled. Merging PR...${NC}"
  ...
  if echo "$MERGE_RESPONSE" | jq -e '.merged' > /dev/null; then
    echo -e "${GREEN}✓${NC} PR merged successfully!"
  else
    echo -e "${YELLOW}⚠️  Auto-merge failed. Manual approval required.${NC}"
    echo -e "   Reason: $(echo "$MERGE_RESPONSE" | jq -r '.message')"
  fi
else
  echo ""
  echo -e "${YELLOW}ℹ️  Auto-merge disabled. PR requires manual approval.${NC}"
...
echo ""
echo -e "${GREEN}╔════════════════════════════════════════════════════════════╗${NC}"
echo -e "${GREEN}║  ✓ Full catalog re-index complete!                        ║${NC}"
echo -e "${GREEN}╚════════════════════════════════════════════════════════════╝${NC}"
echo ""
echo -e "📊 View report: ${BLUE}codex-report.json${NC}"
